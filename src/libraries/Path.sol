...
library Path {
    ...
    function decodeFirstV4Pool(bytes memory path) internal pure returns (PoolKey memory) {
        ...
        return PoolKey({currency0: currency0, currency1: currency1, fee: fee, tickSpacing: int24(tickSpacing), hooks: IHooks(hooks)});
    }
    ...
    function toV4Pool(bytes memory path) internal pure returns (address token0, uint24 fee, uint24 tickSpacing, address hooks, address token1) {
